                 ...
                 testlevel = 0.05, ...)
{
    ...
    lcl[[1]] <- ## hack --> eval(.) works also when call is source()d ...
        switch(lfitname,
               "i.lm" = quote(i.lm),
               "i.glm" = quote(i.glm),
               "i.multinomial" = quote(i.multinomial),
               "i.polr" = quote(i.polr), 
               "i.smooth" = quote(i.smooth), ## ??
               "i.survreg" = quote(i.survreg),
               ## default:
               as.name(lfitname))
                ...
                if(lpls=="weights") {
                    ...
                    if (lpllevel>0) { # plot on weights
                        ...
                        if (length(lwgts)!=ln)
                            ...
                            lwg[lwg<=0] <- NA
                            lraw <- lrabs*sqrt(lwg)
                            i.plotlws(lwg,lraw, "weight",
                                      paste("|",lstrname,"|*sqrt(w)"),main,outer.margin, cex.title,
...
simresiduals.glm <- function(object, nrep, resgen=NULL, glm.restype="deviance")
{
    ...
    lcl <- call("glm", formula=lform, data=as.name("ldata"),
                family=object$family, start=object$coef, model=FALSE, y=FALSE)
    lfam <- object$distrname
    ...
    if(lfam%in%c("binomial","quasibinomial")) {
        ...
        resgen <- if(NCOL(ly)==1) function(x) rbinom(x, 1, lfit)
        else
            function(x) {
                lnbin <- ly[,1]+ly[,2]
        ...
        if (lfam%in%c("poisson","quasipoisson")) 
            resgen <- function(x) rpois(x, lfit)  else {
...
## ==========================================================================
simresiduals.default <- function(object, nrep, resgen=NULL)
...
plresx <-
              ...
              ...)
    {
        ...
        ## check vars, find them in call$data
        dvars <- names(ldata)
        if (length(vars)) {
            lmv <- match(vars,dvars,nomatch=0)
            if (any(lmv==0)) {
                lvm <- vars[lmv==0]
        ...
        ##  --- sequence
        ljseq <- length(sequence) && (!is.na(sequence)) && sequence
        if (ljseq) {
